...
export default function WarRoom({ session, profile }) {
  ...
  const fetchPostsAndReactions = async () => {
    const { data: postsData, error: postsError } = await supabase.from('posts_with_reactions').select('*').order('created_at', { ascending: false });
    if (postsError) { console.error('Error fetching posts:', postsError); } else { setPosts(postsData); }
    const postIds = postsData?.map(p => p.id) || [];
    if (postIds.length > 0) {
      const { data: reactionsData, error: reactionsError } = await supabase.from('post_reactions').select('post_id, reaction_type').eq('profile_id', session.user.id).in('post_id', postIds);
      if (reactionsError) { console.error('Error fetching user reactions:', reactionsError); } else { const reactionsMap = reactionsData.reduce((acc, reaction) => { acc[reaction.post_id] = reaction.reaction_type; return acc; }, {}); setUserReactions(reactionsMap); }
    }
  };
  ...
  const handleReaction = async (postId, reactionType) => {
    setAnimatingReaction({ postId, type: reactionType });
