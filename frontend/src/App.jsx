...
function App() {
  ...
  const setupUser = async (user) => { if (!user) { setProfile(null); setLeague(null); setLoading(false); return; } const { data: userProfile } = await supabase.from('profiles').select('username, active_league_id, hash_marks, avatar_url').eq('id', user.id).single(); setProfile(userProfile); if (userProfile?.active_league_id) { const { data: leagueData } = await supabase.from('leagues').select('*').eq('id', userProfile.active_league_id).single(); setLeague(leagueData); } else { setLeague(null); } setLoading(false); };
          ...
          <ProtectedRoute>
            <MainLayout session={session} profile={profile} league={league} handleLogout={handleLogout} onSwitch={() => setupUser(session.user)}>
              <Routes>
                ...
                <Route path="/paper-football" element={<PaperFootball />} />
